...
Note that training the network becomes more challenging as the number of layers increases.
"""
...
class ESN():
    ...
    def __init__(self, n_inputs, n_outputs, n_reservoir=500, n_layer=1, nonlin = 1,
    ...
    def _update(self, statel, input_pattern, output_pattern, l):
    ...
    def fit(self, inputs, outputs, inspect=False):
        ...
        #self.n_reservoir = np.zeros((self.n_layer,1))
        #statesi[0,:] = zero is IC
        if self.n_layer >= 1:
        ...
        for n in range(1, inputs.shape[0]):
            if self.n_layer==1:
                states1[n,:] = self._update(states1[n-1,:], inputs_scaled[n,:],teachers_scaled[n-1,:],0)
            else:
                if self.n_layer>=1:
                    states1[n,:] = self._update(states1[n-1,:], inputs_scaled[n,:],np.zeros(teachers_scaled[n-1,:].shape),0)
                if self.n_layer>=2 and self.n_layer != 2:
                    states2[n,:] = self._update(states2[n-1,:], states1[n,:], np.zeros(teachers_scaled[n-1,:].shape),1)
                if self.n_layer==2:
                    states2[n,:] = self._update(states2[n-1,:], states1[n,:], teachers_scaled[n-1,:],1)
                if self.n_layer>=3 and self.n_layer != 3:
                    states3[n,:] = self._update(states3[n-1,:], states2[n,:], np.zeros(teachers_scaled[n-1,:].shape),2)
                if self.n_layer==3:
                    states3[n,:] = self._update(states3[n-1,:], states2[n,:], teachers_scaled[n-1,:],2)
                if self.n_layer>=4 and self.n_layer != 4:
                    states4[n,:] = self._update(states4[n-1,:], states3[n,:], np.zeros(teachers_scaled[n-1,:].shape),3)
                if self.n_layer==4:
                    states4[n,:] = self._update(states4[n-1,:], states3[n,:], teachers_scaled[n-1,:],3)                    
                if self.n_layer >=5:
                    print('why need > 4 layers?')
        ...
        if self.n_layer == 1:
            ...
            if self.nonlin == 1:
                extended_states = np.hstack((states1, states2, states3, states4, inputs_scaled))
                        ...
                        extended_states = np.hstack((states1, states2, states3,states4, states1**2, states2**2, states3**2, states4**2, inputs_scaled))
            else:
                print('why need nonlin > 2?')    
            self.laststate1= states1[-1,:]
            self.laststate2 = states2[-1, :]
    ...
    def predict(self, inputs):
        ...
        for n in range(n_samples):
            if self.n_layer == 1:
                states1[n+1,:] = self._update(states1[n, :], inputs[n + 1, :], outputs[n, :],0)
            else:
                if self.n_layer >= 1:
                    states1[n+1,:] = self._update(states1[n, :], inputs[n + 1, :], np.zeros(outputs[n, :].shape),0)
                if self.n_layer>=2 and self.n_layer != 2:
                    states2[n + 1, :] = self._update(states2[n,:], states1[n+1,:], np.zeros(outputs[n, :].shape),1) 
                if self.n_layer==2:
                    states2[n + 1, :] = self._update(states2[n,:], states1[n+1,:], outputs[n, :],1)
                if self.n_layer >= 3 and self.n_layer !=3:
                    states3[n + 1, :] = self._update(states3[n,:], states2[n+1,:], np.zeros(outputs[n, :].shape),2)
                if self.n_layer == 3:
                    states3[n + 1, :] = self._update(states3[n,:], states2[n+1,:], outputs[n, :],2)
                if self.n_layer >= 4 and self.n_layer !=4:
                    states4[n + 1, :] = self._update(states4[n,:], states3[n+1,:], np.zeros(outputs[n, :].shape),3)
                if self.n_layer == 4:
                    states4[n + 1, :] = self._update(states4[n,:], states3[n+1,:], outputs[n, :],3)
                if self.n_layer >= 5:
                    print('why need > 4 layers?')
